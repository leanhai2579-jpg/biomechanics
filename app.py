...
def stop_record():
    ...
    if video_writer:
        ...
        # 🌟 THÊM LOGIC ĐỒNG BỘ: Tự động quét và cập nhật trường video vào JSON ngay lập tức
        try:
            # Lấy thông tin bài tập hiện tại từ tham số referer (trang trước đó gửi lên)
            referer = request.referrer or ""
            from urllib.parse import urlparse, parse_qs
            parsed_url = urlparse(referer)
            params = parse_qs(parsed_url.query)
            
            # Trích xuất email và loại bài tập đang thực hiện
            email = params.get('email', [''])[0]
            ex_type = params.get('ex_type', ['shoulder'])[0]
            
            if email and current_video_filename:
                # Đọc file lịch sử hiện tại của bệnh nhân lên
                history_data = load_permanent_history(ex_type, email)
                today_key = datetime.datetime.now().strftime("%d/%m")
                
                # Nếu ngày hôm nay đã được tạo bản ghi, bổ sung ngay trường video vào
                if today_key in history_data:
                    history_data[today_key]["video"] = current_video_filename
                    save_permanent_history(ex_type, email, history_data)
                    print(f"=== ĐỒNG BỘ TỰ ĐỘNG: Đã ghi nhận {current_video_filename} vào ngày {today_key} ===")
        except Exception as e:
            print(f"Lỗi đồng bộ ghi file video vào JSON: {e}")
...
@app.route('/save-tutorial-video', methods=['POST'])
def save_tutorial_video():
    """Bác sĩ bấm 'Lưu vào hồ sơ' sau khi quay video mẫu hướng dẫn"""
    global video_writer, current_video_filename
    data = request.json
    email = data.get('email')
    ...
    if not current_video_filename or not os.path.exists(current_video_filename):
        return jsonify({"status": "error", "message": "Không tìm thấy file video hướng dẫn vừa quay."})
        
    users = load_users()
    if email in users and 'exercises' in users[email] and len(users[email]['exercises']) > 0:
        # Lấy bài tập cuối cùng mà bác sĩ vừa mới tạo/gán cho bệnh nhân này
        last_ex_idx = len(users[email]['exercises']) - 1
        
        # Định dạng lại tên file lưu trữ trong static công khai để bệnh nhân có thể truy cập
        safe_email = email.replace('@', '_').replace('.', '_')
        new_filename = f"tutorial_{safe_email}_{last_ex_idx + 1}.mp4"
        dest_path = os.path.join(VIDEO_DIR, new_filename)
        
        # Di chuyển video từ luồng ghi tạm thời vào thư mục public static
        shutil.move(current_video_filename, dest_path)
        
        # Lưu đường dẫn công khai vào cấu trúc object bài tập trong users.json
        users[email]['exercises'][last_ex_idx]['tutorial_video'] = f"/static/videos/{new_filename}"
        save_users(users)
        
        current_video_filename = ""
        return jsonify({"status": "success", "message": "Đã liên kết video hướng dẫn vào bài tập!"})
        
    return jsonify({"status": "error", "message": "Không tìm thấy dữ liệu bài tập."})
